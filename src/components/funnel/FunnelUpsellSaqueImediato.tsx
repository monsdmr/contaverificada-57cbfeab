    ...
    <div className="fixed inset-0 z-[80] bg-[#F0F2F5] overflow-y-auto">
      ...
      <main className="px-3 py-4 max-w-lg mx-auto pb-24">
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm mb-3">
          ...
          <button onClick={onGeneratePix} disabled={isGenerating} className="w-full py-3.5 rounded-xl bg-[#FF6F00] text-white font-bold text-sm tracking-wide hover:brightness-105 transition-all active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-2 shadow-lg shadow-orange-200" style={{ animation: isGenerating ? 'none' : 'ctaPulse 2s ease-in-out infinite' }}>{isGenerating ? (<><Loader2 className="w-4 h-4 animate-spin" />Gerando PIX...</>) : "Receber " + balance + " em 2 Minutos →"}</button>
      ...
      <StickyCtaBar onClick={onGeneratePix} isGenerating={isGenerating} label={"Receber " + balance + " em 2 Minutos"} bgColor="bg-[#FF6F00]" shadowColor="shadow-orange-200" />
    </div>
  );
};
