  ...
  return (
    <div className="fixed inset-0 z-[80] bg-[#F0F2F5] overflow-y-auto pt-6">
      ...
      <main className="px-3 py-4 max-w-lg mx-auto pb-24">
        ...
        <div className="bg-[#FFEBEE] border border-[#EF5350] rounded-xl p-4 mb-3"><div className="flex items-start gap-2"><span className="text-lg leading-none mt-0.5">⚠️</span><div><p className="text-gray-900 text-sm font-bold mb-1">Risco de Reversão Detectado</p><p className="text-gray-700 text-xs leading-relaxed">{firstName ? <><strong className="text-gray-900">{firstName}</strong>, nos</> : "Nos"} últimos 7 dias, <span className="font-bold text-red-700">23% dos saques</span> sem proteção foram revertidos automaticamente.</p></div></div></div>
        <div className="bg-white rounded-xl p-5 shadow-sm mb-3">
          <div className="flex justify-center mb-3"><span className="bg-red-50 text-red-700 text-[10px] font-bold px-3 py-1 rounded-full border border-red-200 animate-pulse">{firstName ? `⚠️ ${firstName.toUpperCase()}, SEU SAQUE ESTÁ DESPROTEGIDO` : "⚠️ SEU SAQUE ESTÁ DESPROTEGIDO"}</span></div>
          <div className="bg-gradient-to-r from-[#FFEBEE] to-[#FCE4EC] rounded-lg p-4 mb-4 border border-red-100"><p className="text-gray-500 text-[10px] text-center mb-1">VALOR EM RISCO SEM PROTEÇÃO</p><p className="text-2xl font-black text-center text-gray-900">{balance}</p><p className="text-red-600 text-[10px] text-center font-semibold mt-1">Pode ser revertido a qualquer momento</p></div>
          <p className="text-gray-600 text-xs font-bold mb-2">O que a proteção garante:</p>
          <div className="space-y-2 mb-4">{["Blindagem contra reversão automática", "Correção de divergência de chave", "Confirmação dupla no BACEN", "Garantia de entrega em até 2 minutos"].map((t, i) => (<div key={i} className="flex items-start gap-2"><Check className="w-4 h-4 text-green-500 mt-0.5 shrink-0" /><p className="text-gray-700 text-xs leading-relaxed"><span className="font-bold">{t}</span></p></div>))}</div>
          <div className="bg-gray-50 rounded-lg p-3 mb-4 border border-gray-100"><div className="flex items-center justify-between mb-1"><span className="text-gray-500 text-[11px]">Taxa de proteção:</span><span className="text-gray-900 text-sm font-bold">R$ 24,87</span></div><div className="flex items-center justify-between"><span className="text-gray-500 text-[11px]">Reembolso automático:</span><span className="text-green-600 text-[11px] font-bold">✓ Em até 2 min</span></div></div>
