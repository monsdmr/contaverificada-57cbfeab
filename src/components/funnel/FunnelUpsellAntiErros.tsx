    ...
    <div className="fixed inset-0 z-[80] bg-white overflow-y-auto">
      ...
      <main className="px-4 py-3 space-y-3 max-w-md mx-auto pb-24">
        ...
        <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
          ...
          <button onClick={onGeneratePix} disabled={isGenerating} className="w-full py-4 rounded-xl bg-emerald-500 text-white font-bold text-base hover:brightness-110 transition-all active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/30" style={{ animation: isGenerating ? 'none' : 'ctaPulse 2s ease-in-out infinite' }}>{isGenerating ? (<><Loader2 className="w-4 h-4 animate-spin" />Gerando PIX...</>) : (<>PROTEGER MEU SAQUE DE {balance}<ChevronRight className="w-4 h-4" /></>)}</button>
