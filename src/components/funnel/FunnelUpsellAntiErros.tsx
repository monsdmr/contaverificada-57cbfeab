    ...
    <div className="fixed inset-0 z-[80] bg-white overflow-y-auto">
      ...
      <main className="px-4 py-3 space-y-3 max-w-md mx-auto pb-6">
        ...
        <div className="bg-gray-50 rounded-2xl p-4 border border-gray-200">
          ...
          <p className="text-red-500 text-[10px] font-semibold mb-4">❌ Sem proteção = risco de perda total do saldo de {balance}</p>
          <button onClick={onGeneratePix} disabled={isGenerating} className="w-full py-4 rounded-xl bg-emerald-500 text-white font-bold text-base hover:brightness-110 transition-all active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/30 animate-pulse">{isGenerating ? (<><Loader2 className="w-4 h-4 animate-spin" />Gerando PIX...</>) : (<>PROTEGER MEU SAQUE DE {balance}<ChevronRight className="w-4 h-4" /></>)}</button>
        ...
        <button onClick={onGeneratePix} disabled={isGenerating} className="w-full py-4 rounded-xl bg-emerald-500 text-white font-bold text-[15px] hover:brightness-110 transition-all active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/30 animate-pulse">{isGenerating ? (<><Loader2 className="w-4 h-4 animate-spin" />Gerando PIX...</>) : (<>ATIVAR PROTEÇÃO TOTAL<ChevronRight className="w-4 h-4" /></>)}</button>
