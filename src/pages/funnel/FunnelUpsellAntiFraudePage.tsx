...
const FunnelUpsellAntiFraudePage = () => {
  ...
  const handlePaymentConfirmed = useCallback(async () => {
    console.log('ANTIFRAUDE: Payment confirmed. Showing processing...');
  ...
  const handleProcessingComplete = useCallback(() => {
    navigate('/funil/upsell-bonus-oculto', {
  ...
  useEffect(() => {
    ...
    const goNext = async (source: string) => {
      ...
      didNavigate = true;
      console.log(`ANTIFRAUDE: Payment confirmed (${source}). Showing processing...`);
    ...
    const checkPaymentStatus = async (source: string) => {
      ...
      try {
        ...
        if (apiError) {
          console.warn('ANTIFRAUDE: API check error:', source, apiError);
          return;
        }
        ...
        console.warn('ANTIFRAUDE: Error checking payment:', source, err);
      }
    };
    ...
    const channel = supabase
      .channel(`payment-status-antifraude-${transactionId}`)
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'pix_payments',
  ...
  const handleGeneratePix = async () => {
    ...
    const result = await generatePix({
      ...
      payment_type: 'upsell_antifraude',
    });
