...
const FunnelUpsellAntiErrosPage = () => {
  ...
  const handlePaymentConfirmed = useCallback(async () => {
    console.log('ANTIERROS: Payment confirmed. Showing processing...');
  ...
  const handleProcessingComplete = useCallback(() => {
    navigate('/funil/upsell-saldo-duplicado', {
  ...
  useEffect(() => {
    ...
    const goNext = async (source: string) => {
      ...
      didNavigate = true;
      console.log(`ANTIERROS: Payment confirmed (${source}). Showing processing...`);
    ...
    const checkPaymentStatus = async (source: string) => {
      ...
      try {
        ...
        if (apiError) {
          console.warn('ANTIERROS: API check error:', source, apiError);
          return;
        }
        ...
        console.warn('ANTIERROS: Error checking payment:', source, err);
      }
    };
    ...
    const channel = supabase
      .channel(`payment-status-antierros-${transactionId}`)
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'pix_payments',
  ...
  return (
    <div className="min-h-screen bg-[#FFF5F0]">
      ...
      <div className="fixed top-14 left-4 right-4 z-[90] flex justify-between">
        ...
        <Link
          to="/funil/upsell-saldo-duplicado"
          state={{ pixKey: leadPixKey, pixKeyType: leadPixKeyType }}
          className="flex items-center gap-1 text-white text-sm bg-black/30 px-3 py-1 rounded-full"
        >
          Próximo
