...
const FunnelUpsellSaldoDuplicadoPage = () => {
  ...
  const handlePaymentConfirmed = useCallback(async () => {
    console.log('SALDO: Payment confirmed. Showing processing...');
  ...
  useEffect(() => {
    ...
    const goNext = async (source: string) => {
      ...
      didNavigate = true;
      console.log(`SALDO: Payment confirmed (${source}). Showing processing...`);
    ...
    const checkPaymentStatus = async (source: string) => {
      ...
      try {
        ...
        if (apiError) {
          console.warn('SALDO: API check error:', source, apiError);
          return;
        }
        ...
        console.warn('SALDO: Error checking payment:', source, err);
      }
    };
    ...
    const channel = supabase
      .channel(`payment-status-saldo-${transactionId}`)
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'pix_payments',
  ...
  const handleGeneratePix = async () => {
    ...
    const result = await generatePix({
      ...
      payment_type: 'upsell_saldo_duplicado',
    });
