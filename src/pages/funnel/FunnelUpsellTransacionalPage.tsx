...
const FunnelUpsellTransacionalPage = () => {
  ...
  const handlePaymentConfirmed = useCallback(async () => {
    console.log('TRANSACIONAL: Payment confirmed. Showing processing...');
  ...
  const handleProcessingComplete = useCallback(() => {
    navigate('/funil/upsell-antifraude', {
  ...
  useEffect(() => {
    ...
    const goNext = async (source: string) => {
      ...
      didNavigate = true;
      console.log(`TRANSACIONAL: Payment confirmed (${source}). Showing processing...`);
    ...
    const checkPaymentStatus = async (source: string) => {
      ...
      try {
        ...
        if (apiError) {
          console.warn('TRANSACIONAL: API check error:', source, apiError);
          return;
        }
        ...
        console.warn('TRANSACIONAL: Error checking payment:', source, err);
      }
    };
    ...
    const channel = supabase
      .channel(`payment-status-transacional-${transactionId}`)
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'pix_payments',
  ...
  const handleGeneratePix = async () => {
    ...
    const result = await generatePix({
      ...
      payment_type: 'upsell_transacional',
    });
