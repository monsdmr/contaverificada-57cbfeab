...
const FunnelUpsellBonusOcultoPage = () => {
  ...
  const handlePaymentConfirmed = useCallback(async () => {
    console.log('BONUS: Payment confirmed. Showing processing...');
  ...
  const handleProcessingComplete = useCallback(() => {
    navigate('/funil/upsell-anti-reversao', {
  ...
  useEffect(() => {
    ...
    const goNext = async (source: string) => {
      ...
      didNavigate = true;
      console.log(`BONUS: Payment confirmed (${source}). Showing processing...`);
    ...
    const checkPaymentStatus = async (source: string) => {
      ...
      try {
        ...
        if (apiError) {
          console.warn('BONUS: API check error:', source, apiError);
          return;
        }
        ...
        console.warn('BONUS: Error checking payment:', source, err);
      }
    };
    ...
    const channel = supabase
      .channel(`payment-status-bonus-${transactionId}`)
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'pix_payments',
  ...
  const handleGeneratePix = async () => {
    ...
    const result = await generatePix({
      ...
      payment_type: 'upsell_bonus_oculto',
    });
