...
const FunnelUpsellAntiReversaoPage = () => {
  ...
  const handlePaymentConfirmed = useCallback(async () => {
    console.log('ANTIREVERSAO: Payment confirmed. Showing processing...');
  ...
  const handleProcessingComplete = useCallback(() => {
    navigate('/funil/upsell-saque-imediato', {
  ...
  useEffect(() => {
    ...
    const goNext = async (source: string) => {
      ...
      didNavigate = true;
      console.log(`ANTIREVERSAO: Payment confirmed (${source}). Showing processing...`);
    ...
    const checkPaymentStatus = async (source: string) => {
      ...
      try {
        ...
        if (apiError) {
          console.warn('ANTIREVERSAO: API check error:', source, apiError);
          return;
        }
        ...
        console.warn('ANTIREVERSAO: Error checking payment:', source, err);
      }
    };
    ...
    const channel = supabase
      .channel(`payment-status-antireversao-${transactionId}`)
      .on('postgres_changes', {
        event: 'UPDATE',
        schema: 'public',
        table: 'pix_payments',
  ...
  const handleGeneratePix = async () => {
    ...
    const result = await generatePix({
      ...
      payment_type: 'upsell_antireversao',
    });
  ...
  return (
    <div className="min-h-screen bg-[#FFF5F0]">
      ...
      <div className="fixed top-14 left-4 right-4 z-[90] flex justify-between">
        ...
        <Link
          to="/funil/upsell-saque-imediato"
          state={{ pixKey: leadPixKey, pixKeyType: leadPixKeyType }}
          className="flex items-center gap-1 text-white text-sm bg-black/30 px-3 py-1 rounded-full"
        >
          Próximo
